...
async function testOraclePreflightReportsBlockingReadinessStates(): Promise<void> {
  ...
  try {
    ...
    if (process.platform === "darwin") {
      // cp is only a required dependency for the apfs-clone strategy, which is darwin-only
      // (see requiredOracleDependencies in runtime.ts); skip this assertion elsewhere.
      const originalCpPath = process.env.PI_ORACLE_CP_PATH;
      try {
